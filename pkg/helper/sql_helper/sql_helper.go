...
func QueryCreateNewTable(nodeType *node_type_model.NodeType) string {
	query := fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s (id text PRIMARY KEY, created_at timestamptz DEFAULT NOW(), created_by text, modified_at timestamptz, modified_by text, deleted_at timestamptz, deleted_by text, ", strcase.ToSnake(nodeType.TID))
